...
function isGreeting(text) {
  ...
  return ["hola","buenas","buenas tardes","buen dia","buenos dias","buenas noches","ok","oki","gracias","sale","va","si","sí","👍","🙂","😊"].includes(t);
}
...
function detectDatePreference(text) {
  ...
  if (t.includes("mes que viene")||t.includes("proximo mes")||t.includes("próximo mes")||t.includes("siguiente mes")) {
...
function datePreferenceToRange(pref) {
  ...
  if (pref?.month && pref?.year) {
    ...
    if (from < now) { from.setTime(now.getTime()); from.setHours(now.getHours()+2,0,0,0); }
    return { from, to };
  }
...
async function huliRequest(method, path, body = null) {
  ...
  const opts = {
    method,
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", id_organization: HULI_ORG_ID },
  };
...
async function huliCreatePatient(patient) {
  ...
  let dob = null;
  const m = String(patient.fechaNacimiento||"").match(/(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})/);
  if (m) { const y = m[3].length===2?`20${m[3]}`:m[3]; dob=`${y}-${m[2].padStart(2,"0")}-${m[1].padStart(2,"0")}`; }
  const body = {
    personal_data: { first_name: firstName, last_name: lastName, ...(dob?{date_of_birth:dob}:{}) },
    contact: { phones: [{ type:"MOBILE", phone_number:phone, id_country:484 }] },
  };
...
function isAllowedSlotPublic(rawTime) {
  ...
  return ALLOWED_WEEKDAYS.has(weekday) && minutes>=MIN_SLOT_MINUTES && minutes<=MAX_SLOT_MINUTES;
}
...
async function huliGetSlotsInRange(from, to) {
  ...
  const blocks = responseData?.availability?.slots || data?.data?.slots || data?.slots || [];
  ...
  console.log(`Huli public: blocks=${blocks.length} slots_utiles=${slots.length}`);
  if (slots.length) console.log("primeros 3:", JSON.stringify(slots.slice(0,3)));
...
async function callOAI(messages, temp=0.2, tokens=300) {
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body:JSON.stringify({ model:OPENAI_MODEL, messages, temperature:temp, max_tokens:tokens }),
  });
  const raw = await res.text();
  const data = safeJsonParse(raw);
...
Sin explicaciones fuera del JSON.`;
  try {
    const txt = await callOAI(
      [{ role:"system", content:prompt },{ role:"user", content:JSON.stringify({ stage:state.stage, msg, patient:state.patient }) }],
      0.1, 240
    );
...
async function writerReply(state, msg) {
  ...
  const reply = await callOAI([{ role:"system", content:SYSTEM_PROMPT }, ...state.messages], 0.7, 400);
...
async function answerMedicalDoubt(state, msg) {
  ...
  try {
    return await callOAI(
      [{ role:"system", content:prompt },{ role:"user", content:JSON.stringify({ patient:state.patient, message:msg }) }],
      0.5, 280
    );
  } catch {
...
function isPriority(t) {
  t = normalizeText(t);
  return ["parto","cesarea","urgencia","urgente","sangrado abundante","mucho sangrado","dolor intenso","dolor fuerte","dolor insoportable","fiebre","embarazo ectopico","emergencia","no se mueve mi bebe","ya no se mueve mi bebe","no siento movimientos","no siento a mi bebe","dejo de moverse","dejo de sentir a mi bebe","no he sentido movimientos","hoy no he sentido movimientos","no lo siento moverse","mi bebe no se mueve","ya no lo siento","no siento que se mueva mi bebe","casi no se mueve mi bebe"].some((k)=>t.includes(k));
}

function wantsAppointment(t) {
  t = normalizeText(t);
  return ["quiero una cita","quiero cita","agendar","hacer una cita","sacar una cita","disponible","que dia tiene","tiene lugar","puedo ir","cuando puede","necesito cita","quisiera cita","quiero agendar","tiene horarios","que horarios tiene","cuando tiene citas","como saco cita","como agendo","quiero ir a revision","me puede ver"].some((k)=>t.includes(k));
}

function wantsPrice(t) {
  t = normalizeText(t);
  return ["precio","cuanto cuesta","costo","cuanto sale","cuanto cobra","que precio tiene"].some((k)=>t.includes(k));
}

function wantsInfo(t) {
  t = normalizeText(t);
  return ["tengo una duda","una duda","quiero informacion","quisiera informacion","quiero preguntar","tengo una pregunta","informes","me puede orientar","tengo sintomas","tengo molestia","tengo flujo","tengo dolor","tengo sangrado","duda de"].some((k)=>t.includes(k));
}

function asksIfDoctor(t) {
  t = normalizeText(t);
  return ["hablo con el dr","hablo con el doctor","hablo con el dr cid","es el dr cid","es usted el doctor","usted es el doctor","eres el doctor","es el doctor ricardo"].some((k)=>t.includes(k));
}

function wantsDoctorDirect(t) {
  t = normalizeText(t);
  return ["hablar con el doctor","hablar directamente con el doctor","quiero hablar con el doctor","comunicarme con el doctor","quiero hablar con el dr","me puede pasar con el doctor","me comunicas con el doctor","quiero hablar directamente con el dr","pasame con el doctor","pásame con el doctor"].some((k)=>t.includes(k));
}

function cantMakeIt(t) {
  t = normalizeText(t);
  return ["no puedo","no me queda","otro horario","otra hora","no me acomoda","diferente","ninguno","mas tarde","mas temprano","otro dia","otra fecha"].some((k)=>t.includes(k));
}
...
function looksLikeRealName(text) {
  ...
  const blocked = ["precio","cuesta","costo","cita","consulta","duda","horario","doctor","ultrasonido","embarazo","dolor","sangrado","quiero","tengo","para","pero","motivo","informacion","pregunta","mes que viene","proximo mes","siguiente mes","en 2 semanas"];
  if (blocked.some((w)=>n.includes(w))) return false;
  const words = clean.split(/\s+/).filter(Boolean);
  if (words.length<2||words.length>5) return false;
  const particulas = new Set(["de","del","la","las","los","el","y"]);
  return words.every((w)=>particulas.has(w.toLowerCase())||(/^[A-Za-zÀ-ÿÑñ]+$/.test(w)&&w.length>=2));
}
...
function extractData(state, text, fromPhone) {
  ...
  if (!state.patient.nombre && (state.stage==="datos"||state.stage==="datos_manual") && nextMissingField(state)==="nombre" && !isGreeting(clean) && looksLikeRealName(clean)) {
    state.patient.nombre = clean;
  }
  const motivoKw = ["embarazo","revision","ultrasonido","dolor","sangrado","infeccion","planificacion","quiste","mioma","papanicolaou","pap","menstruacion","regla","colposcop","chequeo","general","flujo","comezon","ardor"];
  if (!state.patient.motivo && motivoKw.some((k)=>n.includes(k))) state.patient.motivo = clean.slice(0,200);
  if (!state.patient.motivo && (state.stage==="datos"||state.stage==="datos_manual") && nextMissingField(state)==="motivo" && !isGreeting(clean)) {
...
async function notifyDoctor(type, state, extra="") {
  if (!TWILIO_ACCOUNT_SID||!TWILIO_AUTH_TOKEN||DOCTOR_WHATSAPP.includes("XXXXXXXXXX")) {
  ...
  try {
    ...
    const resp = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${TWILIO_ACCOUNT_SID}/Messages.json`,{
      ...
      body: new URLSearchParams({ From:TWILIO_FROM, To:DOCTOR_WHATSAPP, Body:bodies[type] }).toString(),
    });
    console.log(resp.ok?`✅ Doctor notificado: ${type}`:`❌ Error Twilio: ${await resp.text()}`);
  } catch(e) { console.error("notifyDoctor error:", e); }
}

// ── SLOTS ─────────────────────────────────────────────────────────────────────

function formatSlots(slots) {
  return (slots||[]).slice(0,3).map((s,i)=>`${i+1}. ${s.date_l10n} a las ${s.time_l10n}`).join("\n");
}

function detectSlotChoice(msg, cls, slots) {
  if (!slots?.length) return null;
  const t = normalizeText(msg);
  if (t==="1"||t.startsWith("1 ")||t.startsWith("el 1")||t.includes("primera")||t.includes("primer")) return slots[0]||null;
  if (t==="2"||t.startsWith("2 ")||t.startsWith("el 2")||t.includes("segunda")||t.includes("segundo")) return slots[1]||null;
  if (t==="3"||t.startsWith("3 ")||t.startsWith("el 3")||t.includes("tercera")||t.includes("tercer")) return slots[2]||null;
  ...
  for (const slot of slots) {
    ...
    if (t.includes(hhmm)||t.includes(normalizeText(slot.time_l10n))||t.includes(normalizeText(slot.date_l10n))) return slot;
  }
  return null;
}
...
async function pedirDatosYEsperarManual(state) {
  ...
  if (q) { state.stage="datos_manual"; return `Con mucho gusto 😊\n\nPara poder agendarte necesito algunos datos.\n${q}`; }
  return await confirmarEsperaManual(state);
}

async function confirmarEsperaManual(state) {
  state.stage = "espera_confirmacion";
  if (!state.flags.notificado) { state.flags.notificado=true; notifyDoctor("CITA_MANUAL",state).catch(console.error); }
...
async function ofrecerHorarios(state, datePreference=null) {
  if (!huliDisponible()) { state.stage="datos_manual"; return await pedirDatosYEsperarManual(state); }
  try {
    let slots = [];
    if (datePreference) {
      ...
      if (!slots.length) {
        const fallback = await huliGetSlots(60);
        if (fallback.length) {
      ...
      if (!slots.length) slots = await huliGetSlots(60);
    }
    if (!slots.length) {
      state.stage="datos_manual";
      return `Claro 😊 En este momento no me está mostrando espacios disponibles.\n\nSi gustas, te dejo registrada para que el consultorio te confirme el espacio más próximo.\n\n${buildNextDataQuestion(state)}`;
    ...
    const intro = datePreference ? `Para ${datePreference.text} tengo disponibilidad en:` : "Tengo disponibilidad en:";
    return `${intro}\n${formatSlots(state.slots)}\n\n¿Cuál te queda mejor? (responde 1, 2 o 3)`;
  } catch(e) {
...
async function ofrecerHorariosAlternativos(state, msg, cls) {
  ...
  const newDatePref = detectDatePreference(msg)||(cls?.date_preference?detectDatePreference(cls.date_preference):null);
  if (newDatePref) { state.datePreference=newDatePref; return await ofrecerHorarios(state,newDatePref); }
  if (!huliDisponible()) { state.stage="datos_manual"; return await pedirDatosYEsperarManual(state); }
  try {
    ...
    if (rest.length>=2) { state.slots=rest.slice(0,3); return `Claro 😊 También tengo:\n${formatSlots(state.slots)}\n\n¿Cuál te queda mejor? (responde 1, 2 o 3)`; }
    allSlots = await huliGetSlots(45);
    rest = allSlots.filter((s)=>!currentIds.has(s.source_event));
    if (rest.length>=2) { state.slots=rest.slice(0,3); return `Claro 😊 Un poco más adelante tengo:\n${formatSlots(state.slots)}\n\n¿Cuál te queda mejor? (responde 1, 2 o 3)`; }
    state.stage="datos_manual"; return await pedirDatosYEsperarManual(state);
  } catch(e) { console.error("❌ Huli alt error:", e.message); state.stage="datos_manual"; return await pedirDatosYEsperarManual(state); }
}
...
async function agendarCita(state) {
  ...
  try {
    ...
    if (!patientFileId) { patientFileId = await huliCreatePatient(state.patient); console.log("✅ Paciente creado:", patientFileId); }
    else { console.log("✅ Paciente existente:", patientFileId); }
    const appt = await huliCreateAppointment(slot, patientFileId, state.patient.motivo);
    console.log("✅ Cita creada:", appt?.id||appt);
    if (!state.flags.notificado) { state.flags.notificado=true; notifyDoctor("CITA_HULI",state).catch(console.error); }
...
async function route(state, msg, fromPhone, cls) {
  ...
  const datePref = detectDatePreference(msg)||(cls?.date_preference?detectDatePreference(cls.date_preference):null);
  ...
  if (state.stage==="idle"&&(asksIfDoctor(msg)||cls?.asks_if_doctor)) return buildDoctorIdentityReply();
  if (state.stage==="idle"&&isGreeting(msg)) return buildWelcomeMessage();
  ...
  if (state.stage==="duda") {
    if (wantsAppointment(msg)||cls?.wants_appointment) {
      if (!state.patient.motivo) { state.stage="motivo"; return `Claro, con gusto te ayudo a agendar 😊\n\n¿Me puedes decir brevemente el motivo de la consulta?`; }
      state.stage="horario"; return await ofrecerHorarios(state,state.datePreference||null);
    }
    ...
    if (isGreeting(msg)) return "Claro 😊 Cuéntame tu duda o qué síntomas tienes, y con gusto te apoyo.";
    return await answerMedicalDoubt(state, msg);
  }

  if (state.stage==="espera_confirmacion") return `Ya tenemos tus datos y el consultorio te contactará pronto para confirmar tu horario 😊\n\nCualquier duda, aquí me quedo al pendiente.`;
  ...
  if ((wantsPrice(msg)||cls?.wants_price)&&!state.flags.pidioPrecio&&!state.patient.motivo) {
    state.flags.pidioPrecio=true; state.stage="precio_q";
  ...
  if ((wantsPrice(msg)||cls?.wants_price)&&!state.flags.yaDimosPrecio&&state.patient.motivo) {
    state.flags.yaDimosPrecio=true; state.stage="horario";
  ...
  if (wantsAppointment(msg)||cls?.wants_appointment) {
    if (datePref&&!state.patient.motivo) { state.stage="motivo"; return `Claro, con gusto te ayudo a agendar para ${datePref.text} 😊\n\n¿Me puedes decir brevemente el motivo de la consulta?`; }
    if (!state.patient.motivo) { state.stage="motivo"; return `Claro, con gusto te ayudo a agendar 😊\n\n¿Me puedes decir brevemente el motivo de la consulta?`; }
    state.stage="horario"; return await ofrecerHorarios(state,state.datePreference||null);
  }
  ...
  if (state.stage==="horario") {
    if (datePref) { state.datePreference=datePref; return await ofrecerHorarios(state,datePref); }
    if (cantMakeIt(msg)||cls?.cant_make_it) return await ofrecerHorariosAlternativos(state,msg,cls);
    const chosen = detectSlotChoice(msg,cls,state.slots);
    if (chosen) {
      state.chosenSlot=chosen;
      const next = buildNextDataQuestion(state);
      if (next) { state.stage="datos"; return `Perfecto, reservamos el ${chosen.date_l10n} a las ${chosen.time_l10n} 😊\n\n${next}`; }
  ...
  if (!OPENAI_API_KEY) return "Claro 😊 Soy el asistente del Dr. Ricardo Cid Trejo. ¿En qué te puedo apoyar?";
  try { return await writerReply(state,msg); }
  catch { return "Claro 😊 Soy el asistente del Dr. Ricardo Cid Trejo. Cuéntame cómo te puedo apoyar."; }
}
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method==="POST"&&(req.url==="/whatsapp"||req.url==="/whatsapp/")) {
      ...
      let cls = null;
      try { cls = await classify(state,msg); } catch(e) { console.error("Classifier error:",e.message); }
